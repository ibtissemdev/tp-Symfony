...
class TeamController extends AbstractController
{
    ...
    public function organigramme()
    {
        ...
        foreach ($users as $aUser) {
            ...
            $aTeam['key'] = $aUser->getFirstname() . " " . $aUser->getLastname();
            ...
            foreach ($aTeam['position'] as $aLabel) {
                if ($aTeam['id_label'] = $aLabel->getId()) {
        ...
        //   echo "<pre>",  print_r($newLabel,1 ), '<pre>';
    ...
    /**
     * @Route("/team/membreTeam/{id}", name="app_team_membreTeam/{id}")
     */

    public function membreTeam($id) :Response
    {
        

        
        $membre = $this->getDoctrine()->getRepository(Team::class)->findOneBy([
            'id' => $id,
       
        ]); //Récupérer une collection d'objets


        $element['id'] = $membre->getId();
        $element['age'] = $membre->getAge();
        $element['adresse'] = $membre->getAdresse();
        $element['tel'] = $membre->getTel();
        $element['mail'] = $membre->getMail();
        $element['firstname'] = $membre->getFirstname();
        $element['lastname'] = $membre->getLastname();
        $element['key'] = $membre->getFirstname() . " " . $membre->getLastname();
        $element['supHierarchique'] = $membre->getSupHierarchique();
        $element['photo'] = $membre->getPhoto();
        $element['position'] = $membre->getPositions();

        foreach ($element['position'] as $aLabel) {
            if ($element['id_label'] = $aLabel->getId()) {

                $element['label'] = $aLabel->getLabel();
            }
        }


        return $this->render('team/membre.html.twig', [
            'message' => 'salut',
            'user'=> $element
        ]);
    }
